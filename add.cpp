...
std::int32_t main()
{
    ...
    std::int64_t A_ID = 0;
    std::int64_t A_dim[] = {b, m, n};
    std::int64_t A_stride[] = {m * n, n, 1};
    ...
    std::int64_t B_ID = 1;
    std::int64_t B_dim[] = {b, m, n};
    std::int64_t B_stride[] = {m * n, n, 1};
    ...
    cudnnBackendDescriptor_t add_desc;
    checkCudnnErr(cudnnBackendCreateDescriptor(CUDNN_BACKEND_POINTWISE_DESCRIPTOR, &add_desc));

    cudnnPointwiseMode_t mode = CUDNN_POINTWISE_ADD;
    cudnnDataType_t compute_data_type = CUDNN_DATA_FLOAT;

    checkCudnnErr(cudnnBackendSetAttribute(add_desc, CUDNN_ATTR_POINTWISE_MODE, CUDNN_TYPE_POINTWISE_MODE, 1, &mode));
    checkCudnnErr(cudnnBackendSetAttribute(add_desc, CUDNN_ATTR_POINTWISE_MATH_PREC, CUDNN_TYPE_DATA_TYPE, 1, &compute_data_type));

    checkCudnnErr(cudnnBackendFinalize(add_desc));

    cudnnBackendDescriptor_t op_desc;
    checkCudnnErr(cudnnBackendCreateDescriptor(CUDNN_BACKEND_OPERATION_POINTWISE_DESCRIPTOR, &op_desc));

    checkCudnnErr(cudnnBackendSetAttribute(op_desc, CUDNN_ATTR_OPERATION_POINTWISE_PW_DESCRIPTOR, CUDNN_TYPE_BACKEND_DESCRIPTOR, 1, &add_desc));
    checkCudnnErr(cudnnBackendSetAttribute(op_desc, CUDNN_ATTR_OPERATION_POINTWISE_XDESC, CUDNN_TYPE_BACKEND_DESCRIPTOR, 1, &A_desc));
    checkCudnnErr(cudnnBackendSetAttribute(op_desc, CUDNN_ATTR_OPERATION_POINTWISE_BDESC, CUDNN_TYPE_BACKEND_DESCRIPTOR, 1, &B_desc));
    checkCudnnErr(cudnnBackendSetAttribute(op_desc, CUDNN_ATTR_OPERATION_POINTWISE_YDESC, CUDNN_TYPE_BACKEND_DESCRIPTOR, 1, &C_desc));

    checkCudnnErr(cudnnBackendFinalize(op_desc));
    ...
    checkCudnnErr(cudnnBackendSetAttribute(op_graph, CUDNN_ATTR_OPERATIONGRAPH_OPS, CUDNN_TYPE_BACKEND_DESCRIPTOR, 1, &op_desc));
    ...
    Tensor<float> A_gpu(b * m * n);
    Tensor<float> B_gpu(b * m * n);
